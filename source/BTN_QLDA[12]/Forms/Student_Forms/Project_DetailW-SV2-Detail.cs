...
namespace BTN_QLDA_12_.Forms.Student_Forms
{
    public partial class Project_DetailW_SV2_Detail : Form
    {
        ...
        private void LoadInformation()
        {
            ...
            Projects project = _context.Projects.Where(p => p.ProjectID == _Account.TopicID).FirstOrDefault();
            List<ProjectMembers> pm = _context.ProjectMembers.Where(p => p.ProjectID == project.ProjectID).ToList();
            lblLectureName.Text = lecture.FullName;
            lblMaxStudent.Text = _Account.MaxStudents.ToString() + "/" + pm.Count();
