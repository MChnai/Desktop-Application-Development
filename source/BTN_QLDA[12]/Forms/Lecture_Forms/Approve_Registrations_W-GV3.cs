    ...
    public partial class Approve_Registrations_W_GV3 : Form
    {
        ...
        private void xemBảngĐiểmSinhViênToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (lvRequest.SelectedItems.Count > 0)
            {
                ...
                int stuID = GetStudentID(item.SubItems[1].Text);
                User student = _context.users
                                    .Where( u => u.UserId == stuID)
                                    .FirstOrDefault();
                var studentInfo = new Student_Detail_StudyInfo_W_GV3_Detail(_context, student);
                studentInfo.Show();
            }
        }
